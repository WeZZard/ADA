...
- Provide user-type–aware caps: human developers default to 1 minute max; AI agents default to 3 minutes max; allow CLI override.
...
- Inputs (defaults; all tunable via env):
  - `startup_ms`=3000 (`ADA_HOOK_STARTUP_MS`)
  - `per_symbol_ms`=20 (`ADA_HOOK_PER_SYMBOL_MS`)
  - `tolerance_pct`=0.15 (`ADA_HOOK_TIMEOUT_TOLERANCE`)
  - `min_ms`=30000 (`ADA_HOOK_MIN_MS`)
  - `max_ms` from CLI `--hook-timeout-max-ms` or env `ADA_HOOK_MAX_MS`; defaults: human=60000, AI=180000.
  - Hard override `ADA_SCRIPT_LOAD_TIMEOUT_MS` bypasses estimation.

- Formula
  - `base_ms = startup_ms + (symbol_count * per_symbol_ms)`
  - `deadline_ms = clamp(base_ms, min_ms, max_ms)`
  - `timeout_ms = deadline_ms * (1 + tolerance_pct)`
...
- CLI additions (parsed in `tracer_backend/src/controller/main.c:57–84`):
  - `--hook-timeout-max-ms <ms>`: set `max_ms` cap.
  - `--user-type <human|ai>` or `--ai-agent`: select default cap.
- Usage text updated in `tracer_backend/src/controller/cli_usage.c:25–27` to include new options.
- Env variables for non-CLI contexts:
  - `ADA_USER_TYPE=human|ai` or `ADA_AI_AGENT=1`
  - `ADA_HOOK_MAX_MS=<ms>`
  - `ADA_HOOK_STARTUP_MS`, `ADA_HOOK_PER_SYMBOL_MS`, `ADA_HOOK_TIMEOUT_TOLERANCE`, `ADA_HOOK_MIN_MS`
  - `ADA_SCRIPT_LOAD_TIMEOUT_MS` (hard override)
